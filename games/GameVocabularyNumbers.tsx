...
<script>
    // ✅ PATCH 1: Lấy origin thật từ parent để blob iframe gọi đúng /api/tts
    ...
    function handleCorrect(wordObj) {
        ...
        // ✅ PATCH 2: Phát tiếng Việt khi drop đúng
    ...
    // ✅ PATCH 3: Thay Google Translate TTS (bị block CORS) bằng /api/tts proxy
    // Dùng SITE_ORIGIN vì blob iframe không thể dùng relative URL
    function speak(text) {
        if (!text) return;
        if (window.speechSynthesis) window.speechSynthesis.cancel();
        const url = SITE_ORIGIN + '/api/tts?text=' + encodeURIComponent(text.trim()) + '&lang=vi';
