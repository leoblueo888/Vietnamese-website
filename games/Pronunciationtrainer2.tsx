...
function addTone(word: string, tone: string) {
  const toneMap: Record<string, Record<string, string>> = {
    ...
    'au': { huyền: 'àu', sắc: 'áu', hỏi: 'ảu', ngã: 'ãu', nặng: 'ạu' },
...
export function GamePronunciationTrainer2() {
  ...
  const playCorrectSound = (text: string) => {
    ...
    audio.play().catch(e => console.error("Native Audio Playback failed:", e));
  };
    ...
    return (
      <div className="h-full w-full bg-[#050508] text-cyan-50 flex items-center justify-center p-4 lg:p-6 relative overflow-hidden">
        ...
        <div className="max-w-xl w-full bg-white/[0.03] border border-white/10 rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-12 backdrop-blur-3xl shadow-2xl flex flex-col items-center text-center animate-in zoom-in-95 duration-700">
          ...
          <h1 className="text-xl lg:text-3xl font-black tracking-tighter uppercase italic leading-tight mb-4 px-2">
            {t.startTitle}
          </h1>
          ...
          <div className="w-full space-y-4">
            <div className="flex gap-2 justify-center">
              {(['en', 'ru'] as const).map((l) => (
                <button key={l} onClick={() => setLang(l)} className={`px-6 py-2 rounded-xl font-bold border transition-all text-xs flex items-center gap-2 ${lang === l ? 'bg-cyan-500 border-cyan-400 text-black' : 'bg-white/5 border-white/10 text-white/40'}`}>
                   {l.toUpperCase()}
                </button>
              ))}
            </div>
            <button onClick={() => setGameStarted(true)} className="w-full py-4 lg:py-5 bg-white text-black rounded-2xl lg:rounded-[2rem] font-black uppercase tracking-[0.2em] text-xs lg:text-sm hover:bg-cyan-400 transition-all shadow-lg active:scale-95">
              {t.startBtn}
            </button>
          </div>
        </div>
    ...
    <div className="h-full w-full bg-[#050508] text-cyan-50 flex flex-col overflow-hidden">
      ...
      <header className="h-16 flex items-center justify-between px-4 lg:px-8 border-b border-white/10 bg-[#0a0a0f]/80 backdrop-blur-md z-40 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[10px] sm:text-xs lg:text-lg font-black tracking-tighter uppercase italic whitespace-nowrap">
            PRONUNCIATION <span className="text-cyan-400">TRAINER 2</span>
          </h1>
        </div>
        <nav className="hidden lg:flex bg-white/5 rounded-xl p-1 border border-white/10">
          {menuItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`px-4 py-1 rounded-lg text-[10px] font-mono uppercase tracking-widest transition-all ${activeTab === item.id ? 'bg-cyan-500 text-black font-bold' : 'text-white/40 hover:text-white'}`}>
              {item.label}
            </button>
          ))}
        </nav>
        <div className="text-[10px] font-mono text-cyan-500/60 uppercase lg:hidden">
          {menuItems.find(i => i.id === activeTab)?.mobLabel || activeTab}
        </div>
      </header>
        ...
        <section className="h-[75vh] lg:h-auto lg:flex-1 bg-white/[0.02] border-b lg:border border-white/10 lg:rounded-[2rem] flex flex-col overflow-hidden shadow-2xl backdrop-blur-sm z-10">
          <div className="p-3 border-b border-white/10 bg-white/[0.03]">
            <div className="flex lg:hidden overflow-x-auto gap-2 mb-3 pb-1 no-scrollbar">
              {menuItems.map((item) => (
                <button key={`m-nav-${item.id}`} onClick={() => setActiveTab(item.id)} className={`px-4 py-1.5 rounded-full text-[9px] font-mono uppercase whitespace-nowrap border ${activeTab === item.id ? 'bg-cyan-500 border-cyan-400 text-black' : 'bg-white/5 border-white/10 text-white/40'}`}>
                  {item.mobLabel}
                </button>
              ))}
            </div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1 max-h-20 lg:max-h-24 overflow-y-auto custom-scrollbar">
                  {INITIAL_CONSONANTS.map((c, idx) => (
                    <button key={`p-${idx}`} onClick={() => setPrefix(c)} className={`px-2.5 py-1 rounded text-[9px] font-mono border min-w-[32px] ${prefix === c ? 'bg-cyan-500/20 border-cyan-400 text-cyan-300' : 'bg-white/5 border-white/5 text-white/30'}`}>
                      {c || '-'}
                    </button>
                  ))}
                </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 lg:p-6 space-y-3 lg:space-y-4 custom-scrollbar">
            {currentLibrary.map((family, fIdx) => (
              <div key={`f-${fIdx}`} className="bg-white/[0.01] p-2 lg:p-3 rounded-2xl border border-white/5 flex items-center gap-3">
                <div className="w-10 lg:w-16 shrink-0 text-[8px] lg:text-[10px] font-mono text-cyan-400 font-bold border-r border-white/10 pr-2 uppercase">
                  {family.label}
                </div>
                <div className="flex-1 grid grid-cols-6 gap-1 lg:gap-2">
                  {family.vowels.map((item, vIdx) => {
                    ...
                    return (
                      <button 
                        key={`v-${vIdx}`} 
                        onClick={() => setSelectedWord({ ...item, base: family.base })} 
                        className={`h-8 lg:h-10 flex items-center justify-center rounded-md lg:rounded-xl transition-all border text-[9px] lg:text-xs font-mono truncate px-0.5 ${isSelected ? 'bg-cyan-500 border-cyan-400 text-black font-bold shadow-lg scale-105' : 'bg-white/5 border-white/10 text-white/60'}`}
                      >
                        {display}
                      </button>
                    );
              ...
              <div className="flex items-center lg:items-start justify-between w-full mb-1 lg:mb-12 relative z-30">
                <div className="flex flex-col flex-1 overflow-hidden mr-4">
                  ...
                  {!isLoadingDetails && wordDetails.meaning && (
                    <div className="text-cyan-400 font-mono text-[9px] lg:text-sm uppercase tracking-[0.1em] lg:tracking-[0.2em] animate-in fade-in slide-in-from-left-2 duration-300 line-clamp-1">
                      {wordDetails.meaning}
                    </div>
                  )}
                </div>
                ...
                {!isLoadingDetails && wordDetails.combo ? (
                  <>
                    <div className="flex items-center justify-between mb-1 lg:mb-0">
                      <div className="text-[7px] lg:text-[11px] font-mono text-cyan-400/40 uppercase tracking-[0.3em]">
                        {t.contextTitle}
                      </div>
                      <button 
                    ...
                    <div className="text-[10px] lg:text-xl font-medium text-white/90 leading-tight lg:leading-relaxed italic border-l-2 border-cyan-500/30 pl-3 lg:pl-6 line-clamp-2">
                      "{wordDetails.combo}"
                    </div>
                    
                    {wordDetails.comboMeaning && (
                      <div className="text-[8px] lg:text-sm font-mono text-cyan-400/60 leading-tight lg:leading-relaxed pl-3 lg:pl-6 mt-0.5 lg:mt-0 line-clamp-1">
                        {wordDetails.comboMeaning}
                      </div>
                    )}
                  </>
                  ...
                  <div className="py-2 text-[8px] font-mono text-white/10 uppercase tracking-widest text-center">
      ...
      <footer className="h-12 flex items-center justify-between px-4 lg:px-8 border-t border-white/5 bg-[#050508] z-40 shrink-0">
        <button 
          ...
          className="text-[8px] lg:text-[10px] font-mono text-white/30 hover:text-cyan-400 uppercase tracking-widest flex items-center gap-2 transition-colors"
        >
          <X size={14} /> {t.backBtn}
        </button>
