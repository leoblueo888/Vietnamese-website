    ...
    return (
      <div className="w-full h-full bg-slate-50 flex items-center justify-center p-4">
        <div className="w-full max-w-xl bg-white rounded-[3rem] shadow-2xl p-10 text-center border-[10px] border-white">
          ...
          }} className="w-full py-6 bg-emerald-600 text-white rounded-[2rem] font-black text-2xl shadow-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-3">
      ...
      <div className="w-full h-full max-w-6xl bg-white md:rounded-[3rem] flex flex-col md:flex-row overflow-hidden shadow-2xl">
        <div className="h-[20vh] md:h-full md:w-1/3 bg-blue-50/50 p-4 md:p-10 flex flex-row md:flex-col items-center justify-between border-b md:border-r border-blue-100 shrink-0 z-20">
          ...
          <button 
            ...
            className={`w-14 h-14 md:w-24 md:h-24 rounded-full flex items-center justify-center transition-all shadow-xl ${isRecording ? 'bg-red-500 animate-pulse ring-8 ring-red-100' : 'bg-blue-700 hover:bg-blue-800'}`}
            disabled={isThinking}
          >
        ...
        <div className="flex-1 flex flex-col bg-white overflow-hidden relative">
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-6 bg-blue-50/5 custom-scrollbar">
            ...
            {isThinking && (
              <div className="text-[10px] font-black text-blue-400 animate-pulse ml-4 italic uppercase tracking-widest">
                Linh đang ghi order...
              </div>
            )}
            <div ref={chatEndRef} />
          ...
          <footer className="p-6 md:p-8 bg-white border-t flex gap-3 pb-10">
            <input 
              ...
              className="flex-1 px-6 py-4 bg-slate-50 rounded-[1.5rem] outline-none font-bold focus:bg-white focus:ring-4 ring-blue-50 transition-all shadow-inner" 
              disabled={isThinking}
            />
            <button 
              onClick={() => handleSendMessage(userInput)} 
              className="bg-emerald-600 text-white px-8 rounded-[1.5rem] shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
