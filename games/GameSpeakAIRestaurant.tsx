...
export const GameSpeakAIRestaurant: React.FC<{ character: AIFriend }> = ({ character }) => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const newUserMsg = { role: 'user', text: originalInput, displayedText: originalInput, translation: null, id: userMsgId };
    
    setMessages(prev => {
      const history = [...prev, newUserMsg];
      (async () => {
        try {
          // THAY THẾ BẰNG generateContentWithRetry VÀ GEMINI-2.5-FLASH
