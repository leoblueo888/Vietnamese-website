...
// ĐỒNG BỘ: Sử dụng hệ thống xoay vòng Key để tối ưu tầng Free
...
const LANGUAGES = {
  EN: {
    ...
    ui_start: "SHOP NOW",
    ui_placeholder: "Type to talk to Hanh...",
    ui_status: "Online - Expert Fruit Seller",
    ui_learning_title: "Fruit Market Talk with Hanh",
    welcome_msg: "Dạ, em chào Anh! Anh ghé xem hoa quả sạp em đi ạ. Anh muốn mua trái gì ủng hộ em không ạ? ✨ | Hi! Welcome to my shop. What fruit would you like to buy? ✨",
    systemPromptLang: "English"
  },
  RU: {
    label: "Русский",
    ui_welcome: "Добро пожаловать к Хạnh!",
    ui_start: "КУПИТЬ",
    ui_placeholder: "Поговори с Хань...",
    ui_status: "В сети - Эксперт",
    ui_learning_title: "Общение на рынке",
    welcome_msg: "Dạ, em chào Anh! Mời Anh xem hoa quả ạ. Anh muốn mua trái gì ngon không ạ? ✨ | Здравствуйте! Những trái cây này ngon lắm. Bạn muốn mua gì? ✨",
    systemPromptLang: "Russian"
  }
};

const getSystemPrompt = (targetLangName: string) => {
  return `You are Hạnh (20 years old), a friendly and energetic fruit seller at a local Vietnamese market.
ROLE: You are a fruit seller, NOT a teacher or language assistant.
...
STRICT RULE 3: DO NOT explain grammar. DO NOT give lessons. Just sell fruits.
STRICT RULE 4: Focus on: local specialties (Xoài Cam Lâm, Sầu riêng), prices, sweetness, and weight.
...
export const HanhAIfruitseller: React.FC<{ character: AIFriend }> = ({ character }) => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      // THE CHỐT: Lọc history để AI không bị "nhiễm" thói quen giải thích
  ...
  return (
    <div ref={gameContainerRef} className="w-full h-full bg-slate-900 flex items-center justify-center md:p-4 overflow-hidden relative">
      <div className="w-full h-full max-w-6xl bg-white md:rounded-[3rem] flex flex-col md:flex-row overflow-hidden shadow-2xl">
        <div className="h-[20vh] md:h-full md:w-1/3 bg-emerald-50/50 p-4 md:p-10 flex flex-row md:flex-col items-center justify-between border-b md:border-r border-emerald-100 shrink-0 z-20 shadow-xl">
            ...
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-5 md:p-6 rounded-[2rem] transition-all shadow-sm ${msg.role === 'user' ? 'bg-emerald-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-emerald-100'}`}>
